

### Aspiring Software Engineer 💻

I'm a B.Tech Information Technology student passionate about building real-world applications and solving problems through technology. I enjoy learning new technologies, developing projects, and continuously improving my skills. 🚀

---

## 👩‍💻 About Me

* 🎓 B.Tech Information Technology Student
* 💻 Interested in Full Stack Development
* 🎨 Passionate about UI/UX Design
* 📊 Exploring Data Analytics
* 🚀 Building real-world projects
* 🌱 Learning and improving every day
* 🧩 Interested in problem solving and software development

---

## 🛠️ Skills

### Programming Languages

`C++` `Java` `Python` `JavaScript`

### Frontend

`HTML` `CSS` `React.js`

### Backend

`Spring Boot` `Hibernate/JPA`

### Database

`MySQL` `SQL`

### Tools

`Git` `GitHub` `VS Code` `IntelliJ IDEA` `Figma` `Power BI` `Swagger`

---

## 🚀 Projects

### 📌 Recruitment & Applicant Tracking System

A full-stack recruitment management system developed using React.js, Spring Boot and MySQL.

### 📌 SafeDrive AI

An AI-based road safety solution focused on intelligent monitoring and risk detection.

---

## 📚 Currently Learning

* Full Stack Development
* Data Structures & Algorithms
* Spring Boot & REST APIs
* Data Analytics
* AI & Machine Learning

---

## 🤝 Connect With Me

📧 Email: pandiselvips817631@gmail.com

🔗 LinkedIn: https://www.linkedin.com/in/pandi-selvi-p-7b6513330/



---

### 💡 Build. Learn. Improve. Repeat. 🚀

⭐ Thanks for visiting my profile!
